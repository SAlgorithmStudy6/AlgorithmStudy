...
def solution_diffRow(_limit) -> int:
    global profit
    max_profit = 0
    for first_row_index in range(n-1):
        for first_col_index in range(n):
            ...
            for second_row_index in range(first_col_index + 1, n):
                for second_col_index in range(n):
                    ...
                    profit2 = profit; profit=0
                    max_profit = max(max_profit, profit1+profit2)
    return max_profit

def solution(_limit) -> int:
    profit_diff = solution_diffRow(_limit)
    profit_same = 0
    return max(profit_diff, profit_same)
